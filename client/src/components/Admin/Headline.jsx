...
function Headline() {
    ...
    const handleEditClick = (headline) => {
      setIsEditing(true);
      setCurrentHeadline({
        ...headline,
    ...
    const handleDeleteClick = async (id) => {
      ...
      if (isConfirmed) {
        try {
          ...
          fetchHeadline(); // Refresh the list after deletion
        } catch (error) {
    ...
    const validateForm = () => {
      ...
      if (!currentheadline.hl_text) {
        formErrors.hl_text = "Text is required";
        isValid = false;
      }
      ...
      const headlineData = {
        ...
        try {
          ...
          if (isEditing) {
            ...
            fetchHeadline(); // Refresh the list
            closePopup();
          } else {
            // Create new lead
  ...
  const pageCount = Math.ceil(headline.length / leadsPerPage);
  
  // Pagination logic
  const indexOfLastLead = (currentPage + 1) * leadsPerPage;
  const indexOfFirstLead = indexOfLastLead - leadsPerPage;
  const currentheadlines = leadsPerPage === Infinity ? headline : headline.slice(indexOfFirstLead, indexOfLastLead);
                ...
                <tbody>
                  {currentheadlines.length === 0 ? (
                    ...
                    currentheadlines.map((headline, index) => {
                        console.log(headline, "fdfsdfsdfsdfds");
                      ...
                      >
                       ...
                       {index + 1 + currentPage * leadsPerPage}
                        ...
                        <td className="px-6 py-4 border-b border-gray-200 text-gray-800 font-semibold">
                          {moment(headline.created_date).format("DD MMM YYYY").toUpperCase()}
                        </td>
                        <td className="px-6 py-4 border-b border-gray-200 text-gray-800 font-semibold">
                          <button
                            className="text-blue-500 hover:text-blue-700"
                            onClick={() => handleEditClick(headline)}
                          >
